...
public class MithrilOre extends Block 
{
	...
	public Item getItemDropped(int metadata, Random random, int fortune)
	{
		...
		dropArray = new Item[2];
		
		dropArray[0] = MyCode.mithrilIngot;
		dropArray[1] = Items.iron_ingot;
		
		Random rand = new Random();
		
		int randQuanityDropped = rand.nextInt(dropArray.length);
		
		return dropArray[randQuanityDropped];
	}
